...
impl JwtValidator {
    ...
    pub async fn validate_token(&self, token: &str) -> Result<GitHubClaims, AuthError> {
        ...
        info!(
            "JWT validation successful for repository: {}",
            token_data.claims.repository
        );
    ...
    fn pre_validate_token(&self, token: &str) -> Result<(), AuthError> {
        ...
        if token.to_lowercase().contains("javascript:") || token.to_lowercase().contains("<script")
        {
    ...
    fn validate_github_claims(&self, claims: &GitHubClaims) -> Result<(), AuthError> {
        ...
        if now - claims.iat > self.validation_config.max_token_age {
            warn!(
                "Token too old: age={}s, max={}s",
                now - claims.iat,
                self.validation_config.max_token_age
            );
    ...
    fn perform_security_validations(&self, claims: &GitHubClaims) -> Result<(), AuthError> {
        ...
        if claims.repository.contains("../") || claims.repository.contains("..\\") {
            warn!(
                "Repository name contains path traversal: {}",
                claims.repository
            );
            return Err(AuthError::MalformedToken);
        }
        ...
        if claims.repository_owner.contains('/')
            ...
            || claims.repository_owner.len() > 39
        {
        ...
        if claims.exp - claims.iat > self.validation_config.max_token_lifetime {
            warn!(
                "Token lifetime too long: {}s, max allowed: {}s",
                claims.exp - claims.iat,
                self.validation_config.max_token_lifetime
            );
            return Err(AuthError::TokenLifetimeTooLong);
        }
...
mod tests {
    ...
    mod test_helpers {
        ...
        pub fn long_lifetime_claims() -> GitHubClaims {
            ...
            claims.exp = (now + Duration::seconds(32400)).timestamp(); // 9 hours - exceeds 8 hour default
            claims
        }
    ...
    async fn test_jwt_validation_with_expired_token() {
        ...
        match result.unwrap_err() {
            ...
            other => {
                panic!("Expected TokenExpired or JwtDecodeError for expired token, got: {other:?}")
            }
        }
    }
    ...
    async fn test_jwt_validation_with_suspicious_repository() {
        ...
        let validator = JwtValidator::new_with_url(&format!("{}/.well-known/jwks", server.url()))
            .await
            .unwrap();
        let suspicious_token = test_helpers::create_test_jwt_token_with_claims(
            test_helpers::suspicious_repository_claims(),
        );
    ...
    async fn test_jwt_validation_with_long_lifetime() {
        ...
        match result.unwrap_err() {
            ...
            other => panic!(
                "Expected TokenLifetimeTooLong error for long lifetime token, got: {other:?}"
            ),
        }
    }
    ...
    async fn test_custom_validation_config() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                panic!("Expected successful validation with custom config, got error: {e:?}");
            }
        }
    }
