...
pub async fn handle_client_command(
    ...
    let client = ApiClient::new_with_audience(&host, &scheme, audience);
    ...
    let result: Result<(), crate::error::ClientError> = if is_list_command {
        // Handle List command
        match client
            .list_repositories_with_auto_auth(token.as_deref())
            .await
        {
            Ok(repos_response) => {
        ...
        // Handle Pull command
        match client.get_secrets_with_auto_auth(token.as_deref()).await {
            Ok(secrets_response) => {
                ...
                if github_env && matches!(format, OutputFormat::Env) {
