...
impl Default for GitHubTokenFetcher {
    fn default() -> Self {
        Self::new()
    }
}
...
impl GitHubTokenFetcher {
    ...
    pub async fn fetch_token(&self) -> Result<String, ClientError> {
        self.fetch_token_with_audience(None).await
    }

    /// Fetch GitHub Actions ID token with optional audience
    ///
    /// This method automatically fetches a JWT token from the GitHub Actions environment
    /// using the ACTIONS_ID_TOKEN_REQUEST_URL and ACTIONS_ID_TOKEN_REQUEST_TOKEN environment variables.
    ///
    /// # Arguments
    /// * `audience` - Optional audience for the OIDC token
    ///
    /// # Returns
    /// * `Ok(String)` - The JWT token from GitHub Actions
    /// * `Err(ClientError)` - Error if not in GitHub Actions environment or token fetch fails
    pub async fn fetch_token_with_audience(
        ...
        if let Some(aud) = audience {
            request = request.query(&[("audience", aud)]);
        }
...
impl ApiClient {
    ...
    pub fn new(host: &str, scheme: &str) -> Self {
        Self::new_with_audience(host, scheme, None)
    }

    /// Create a new API client with optional audience parameter for OIDC tokens
    pub fn new_with_audience(host: &str, scheme: &str, audience: Option<String>) -> Self {
        let base_url = format!("{scheme}://{host}");
        let client = Self::create_optimized_client();

        // Validate scheme and provide helpful warnings
        Self::validate_scheme_configuration(host, scheme);

        // Default audience to the server URL if not provided
        let default_audience = audience.unwrap_or_else(|| base_url.clone());

        Self {
            base_url,
            client,
            token_fetcher: GitHubTokenFetcher::new(),
            audience: Some(default_audience),
        }
    }
    ...
    /// Validate scheme configuration and provide helpful warnings
    fn validate_scheme_configuration(host: &str, scheme: &str) {
        use tracing::warn;

        // Warn about potential misconfigurations
        if scheme == "https" && (host.starts_with("localhost") || host.starts_with("127.0.0.1")) {
            warn!("Using HTTPS with localhost/127.0.0.1 may fail if the server doesn't have TLS configured. Consider using --scheme http for local testing.");
        }
        ...
        if scheme == "http"
            ...
            && !host.starts_with("10.")
        {
            warn!("Using HTTP with a remote host may be insecure. Consider using HTTPS in production.");
        }
...
mod tests {
    ...
    async fn test_github_token_fetcher_not_in_github_actions() {
        ...
        let result = token_fetcher.fetch_token().await;
        assert!(matches!(result, Err(ClientError::NotInGitHubActions)));

        let result_with_audience = token_fetcher.fetch_token().await;
        assert!(matches!(
    ...
    fn test_secret_masker_github_actions_detection() {
        ...
        // This should not panic and should output masking commands to stderr
        SecretMasker::mask_secrets(&secrets);
